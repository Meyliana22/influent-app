...
function PaymentConfirmation() {
  ...
  const calculateCosts = () => {
    ...
    if (campaign.contentItems && Array.isArray(campaign.contentItems)) {
      totalPosts = campaign.contentItems.reduce(
        (sum, item) => sum + (parseInt(item.post_count) || 0),
        0
      );
    } else {
  ...
  const handlePayment = async () => {
    try {
      ...
      const campaignResponse = await fetch(
        `https://influent-api-1fnn.vercel.app/api/v1/campaigns/${id}`,
        {
          method: "GET",
      ...
      if (!campaignData) {
        alert("Campaign tidak ditemukan. Silakan coba lagi.");
        return;
      }
      const response = await fetch(
        "https://influent-api-1fnn.vercel.app/api/v1/payments/",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      window.open(result.snap.redirect_url, "_blank");
