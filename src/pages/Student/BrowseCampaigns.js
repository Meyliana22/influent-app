...
function BrowseCampaigns() {
  ...
  useEffect(() => {
    const loadCampaigns = async () => {
      try {
        ...
        // If a single campaign category is selected, prefer the category endpoint
        let response;
        const selectedCategory = (selectedCampaignCategories && selectedCampaignCategories.length === 1) ? selectedCampaignCategories[0] : null;
        if (selectedCategory) {
          response = await campaignService.getCampaignsByCategory(selectedCategory, { limit: itemsPerPage, offset });
        } else {
  ...
  const handleApply = (campaign) => {
    ...
    console.log('Apply for campaign:', campaign.campaign_id);
        ...
        <Box sx={{ marginTop: '72px', background: '#f7fafc', minHeight: 'calc(100vh - 72px)', p: isMobile ? '20px' : '32px' }}>
          ...
          {!loading && campaigns.length > 0 && (
            <Paper sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mt: '40px', p: '20px', background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', flexWrap: isMobile ? 'wrap' : 'nowrap', gap: '16px' }} elevation={0}>
              ...
              <Typography sx={{ fontSize: '0.9rem', color: '#6c757d', order: isMobile ? 1 : 0, textAlign: isMobile ? 'center' : 'auto', flex: isMobile ? '1 0 100%' : 'auto' }}>Showing {campaigns.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0} to {Math.min(currentPage * itemsPerPage, totalItems)} of {totalItems} campaigns</Typography>
              ...
              <Box sx={{ display: 'flex', gap: '8px', order: isMobile ? 3 : 0 }}>
                ...
                <Box sx={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                  {[...Array(Math.ceil(totalItems / itemsPerPage))].map((_, idx) => {
                    ...
                    const isVisible = Math.abs(pageNum - currentPage) <= 1 || pageNum === 1 || pageNum === Math.ceil(totalItems / itemsPerPage);

                    if (!isVisible && (pageNum === 2 || pageNum === Math.ceil(totalItems / itemsPerPage) - 1)) {
                ...
                <Button onClick={() => setCurrentPage(prev => Math.min(Math.ceil(totalItems / itemsPerPage), prev + 1))} disabled={currentPage === Math.ceil(totalItems / itemsPerPage)} sx={{ p: '8px 12px', background: currentPage === Math.ceil(totalItems / itemsPerPage) ? '#e2e8f0' : '#fff', border: '2px solid #e2e8f0', borderRadius: '8px', color: currentPage === Math.ceil(totalItems / itemsPerPage) ? '#a0aec0' : '#667eea', fontWeight: 600, fontSize: '0.9rem', textTransform: 'none', '&:hover': { background: currentPage === Math.ceil(totalItems / itemsPerPage) ? '#e2e8f0' : '#e0e7ff', borderColor: '#667eea' } }}>Next →</Button>
              </Box>
            </Paper>
          )}
        </Box>
