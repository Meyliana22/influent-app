  ...
  try {
    ...
    const apiData = {
      ...
      influencer_category: Array.isArray(campaignData.category) 
        ...
        : (campaignData.category ? JSON.stringify([campaignData.category]) : null),
      has_product: campaignData.hasProduct || false,
      product_name: campaignData.productName || null,
      product_value: campaignData.productValue ? parseInt(campaignData.productValue) : null,
      ...
      price_per_post: campaignData.price_per_post ? parseInt(campaignData.price_per_post) : null,
      min_followers: campaignData.min_followers ? parseInt(campaignData.min_followers) : null,
      ...
      reference_images: campaignData.referenceFiles ? JSON.stringify(campaignData.referenceFiles) : null,
      status: campaignData.status || 'inactive', // Default to inactive (unpaid)
      contentTypes: campaignData.contentItems && campaignData.contentItems.length > 0
        ? JSON.stringify(campaignData.contentItems.map(item => ({
    ...
    console.log('📋 Campaign created with ID:', result?.data?.campaign_id || result?.campaign_id);
...
export const mapApiToFrontend = (apiCampaign) => {
  return {
    ...
    productValue: apiCampaign.product_value ? String(apiCampaign.product_value) : '',
    productDesc: apiCampaign.product_desc || '',
    start_date: apiCampaign.start_date ? apiCampaign.start_date.split('T')[0] : '',
    end_date: apiCampaign.end_date ? apiCampaign.end_date.split('T')[0] : '',
    submission_deadline: apiCampaign.submission_deadline ? apiCampaign.submission_deadline.split('T')[0] : '',
    ...
    price_per_post: apiCampaign.price_per_post ? String(apiCampaign.price_per_post) : '',
    min_followers: apiCampaign.min_followers ? String(apiCampaign.min_followers) : '',
    selectedGender: apiCampaign.selected_gender || '',
    selectedAge: apiCampaign.selected_age || '',
    criteriaDesc: apiCampaign.criteria_desc || '',
    contentItems: apiCampaign.contentTypes && apiCampaign.contentTypes.length > 0
